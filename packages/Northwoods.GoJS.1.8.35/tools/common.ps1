...
$gojsFileName = Join-Path $packageScriptsFolder "go-*.js" | Get-ChildItem -Exclude "*.min.js","*-vsdoc.js" | Split-Path -Leaf
$gojsFileNameRegEx = "go-((?:\d+\.)?(?:\d+\.)?(?:\d+\.)?(?:\d+)).js"
#$gojsFileName -match $gojsFileNameRegEx
#$ver = $matches[1]
